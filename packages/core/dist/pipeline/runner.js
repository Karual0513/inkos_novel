...
export class PipelineRunner {
    ...
    async reviseDraft(bookId, chapterNumber, mode = "rewrite") {
        ...
        try {
            ...
            if (existingFile) {
                await writeFile(join(chaptersDir, existingFile), `# 第${targetChapter}章 ${chapterMeta.title}\n\n${reviseOutput.revisedContent}`, "utf-8");
            }
            // Update truth files
    ...
    async _writeNextChapterLocked(bookId, wordCount, temperatureOverride) {
        ...
        if (output.postWriteErrors.length > 0) {
            process.stderr.write(`[pipeline] ${output.postWriteErrors.length} post-write errors detected, triggering spot-fix before audit\n`);
            const reviser = new ReviserAgent(this.agentCtxFor("reviser", bookId));
        ...
        if (!auditResult.passed) {
            ...
            if (criticalIssues.length > 0) {
                const reviser = new ReviserAgent(this.agentCtxFor("reviser", bookId));
                const reviseOutput = await reviser.reviseChapter(bookDir, output.content, chapterNumber, auditResult.issues, "spot-fix", book.genre);
                if (reviseOutput.revisedContent.length > 0) {
    ...
    async importChapters(input) {
        ...
        try {
            ...
            if (startFrom === 1) {
                ...
                const allText = input.chapters.map((c, i) => `第${i + 1}章 ${c.title}\n\n${c.content}`).join("\n\n---\n\n");
