import { chatWithTools } from "../llm/provider.js";
...
async function executeTool(pipeline, state, config, name, args) {
    switch (name) {
        case "write_draft": {
            const result = await pipeline.writeDraft(args.bookId, args.guidance);
            return JSON.stringify(result);
        }
        case "audit_chapter": {
            const result = await pipeline.auditDraft(args.bookId, args.chapterNumber);
            return JSON.stringify(result);
        }
        case "revise_chapter": {
            const result = await pipeline.reviseDraft(args.bookId, args.chapterNumber, args.mode ?? "rewrite");
            return JSON.stringify(result);
        }
        case "scan_market": {
        ...
        case "get_book_status": {
            const result = await pipeline.getBookStatus(args.bookId);
            return JSON.stringify(result);
        }
        case "read_truth_files": {
            const result = await pipeline.readTruthFiles(args.bookId);
            return JSON.stringify(result);
        }
        case "list_books": {
        ...
        case "write_full_pipeline": {
            ...
            for (let i = 0; i < count; i++) {
                const result = await pipeline.writeNextChapter(args.bookId);
                results.push(result);
            }
        ...
        case "web_fetch": {
            ...
            const text = await fetchUrl(args.url, args.maxChars ?? 8000);
            return JSON.stringify({ url: args.url, content: text });
        }
        case "import_style": {
            const guide = await pipeline.generateStyleGuide(args.bookId, args.referenceText);
        ...
        case "import_canon": {
            const canon = await pipeline.importCanon(args.targetBookId, args.parentBookId);
            return JSON.stringify({
        ...
        case "import_chapters": {
            ...
            const chapters = splitChapters(args.text, args.splitPattern);
