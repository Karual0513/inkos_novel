...
export class WriterAgent extends BaseAgent {
    ...
    async writeChapter(input) {
        ...
        const [storyBible, volumeOutline, styleGuide, currentState, ledger, hooks, chapterSummaries, subplotBoard, emotionalArcs, characterMatrix, styleProfileRaw, parentCanon,] = await Promise.all([
        ...
        const creativeSystemPrompt = buildWriterSystemPrompt(book, genreProfile, bookRules, bookRulesBody, genreBody, styleGuide, styleFingerprint, chapterNumber, "creative");
        ...
        if (ruleViolations.length > 0) {
            process.stderr.write(`[writer] Post-write: ${postWriteErrors.length} errors, ${postWriteWarnings.length} warnings in chapter ${chapterNumber}\n`);
            for (const v of ruleViolations) {
                process.stderr.write(`  [${v.severity}] ${v.rule}: ${v.description}\n`);
            }
        }
        if (aiTellIssues.length > 0) {
            process.stderr.write(`[writer] AI-tell check: ${aiTellIssues.length} issues in chapter ${chapterNumber}\n`);
    ...
    async settle(params) {
        const settlerSystem = buildSettlerSystemPrompt(params.book, params.genreProfile, params.bookRules);
    ...
    async saveChapter(bookDir, output, numericalSystem = true) {
