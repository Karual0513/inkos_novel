...
importCommand
    ...
    .action(async (targetBookIdArg, opts) => {
    try {
        ...
        if (!opts.json)
            log(`Importing canon from "${opts.from}" into "${targetBookId}"...`);
...
importCommand
    ...
    try {
        ...
        if (fromStat.isDirectory()) {
            ...
            chapters = await Promise.all(textFiles.map(async (f) => {
            ...
            if (chapters.length === 0) {
                throw new Error(`No chapters found in ${fromPath}. ` +
