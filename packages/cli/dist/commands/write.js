...
writeCommand
    ...
    try {
        ...
        for (let i = 0; i < count; i++) {
            if (!opts.json)
                log(`[${i + 1}/${count}] Writing chapter for "${bookId}"...`);
...
writeCommand
    ...
    try {
        ...
        if (chapter > 1) {
            ...
            if (restored) {
                if (!opts.json)
                    log(`State restored from chapter ${chapter - 1} snapshot.`);
            }
            else {
                if (!opts.json)
                    log(`Warning: no snapshot for chapter ${chapter - 1}. Using current state.`);
            }
        }
        if (!opts.json)
