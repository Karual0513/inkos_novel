...
import { StateManager, detectChapter, loadDetectionHistory, analyzeDetectionInsights, } from "@actalk/inkos-core";
...
export const detectCommand = new Command("detect")
    ...
    .action(async (bookIdArg, chapterStr, opts) => {
    try {
        ...
        if (!config.detection?.enabled) {
            logError("AIGC detection is not enabled. Set detection.enabled=true in inkos.json, configure detection.apiUrl / detection.apiKeyEnv, then add the matching API key to your .env or shell environment.");
            process.exit(1);
        }
...
async function readChapterContent(bookDir, chapterNumber) {
